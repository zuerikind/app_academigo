import type { de } from "@/messages/de";
import type { Dictionary } from "@/messages/types";

export const en: Dictionary = {
  meta: {
    title: "Academigo — Premium Online Tutoring",
    titleTemplate: "%s | Academigo",
    description:
      "Find expert tutors across 22 school subjects, book lessons, and achieve your learning goals with Academigo.",
  },
  seo: {
    home: {
      title: "Academigo — Online tutoring in Zurich",
      description:
        "Reviewed tutors across 22 school subjects. Flexible packages, online lessons or in-person in Zurich — find your tutor today.",
    },
    teachers: {
      title: "Find tutors",
      description:
        "Browse reviewed Academigo tutors in mathematics, languages, sciences, and more.",
    },
    pricing: {
      title: "Pricing & packages",
      description:
        "Transparent CHF pricing for tutoring — single lessons, bundles, and monthly plans. All prices in Swiss francs.",
    },
    subjects: {
      title: "School subjects",
      description:
        "Expert tutoring across 22 school subjects — from mathematics and languages to geography. Online and in Zurich.",
    },
    about: {
      title: "About Academigo",
      description:
        "Academigo connects Swiss students with reviewed tutors — structured, personal, and digital.",
    },
    teacherProfile: {
      title: "{name} — Tutor",
      description:
        "{name} offers tutoring on Academigo{subjects}. View profile and book a lesson.",
      subjectsSuffix: " in {subjects}",
    },
  },
  common: {
    brand: "Academigo",
    signOut: "Sign out",
    loading: "Loading…",
    getStarted: "Create account",
    logIn: "Log in",
    signUp: "Sign up",
    viewAll: "View all",
    viewProfile: "View profile",
    browseAll: "All teachers",
    comingSoon: "Coming soon",
    placeholderRating: "New on the platform",
    verified: "Verified",
    standard: "Standard",
    approved: "Approved",
    pending: "Pending",
    online: "Online",
    inPerson: "In-person",
    ctaConsultation: "Request free consultation",
    ctaViewTeachers: "View teachers",
    ctaViewPricing: "View pricing",
    ctaApplyTeacher: "Apply as a teacher",
    footer: "© {year} Academigo — Structured tutoring, online and in Zurich.",
    footerTagline:
      "Tutoring across 22 school subjects — online and in Zurich.",
    footerPlatform: "Platform",
    footerContact: "Contact",
    footerLanguage: "Language",
    footerEmail: "Email",
    footerLearnMore: "Learn more on academigo.xyz",
    footerLocation: "Zürich · Online & in-person",
  },
  nav: {
    teachers: "Teachers",
    subjects: "Subjects",
    pricing: "Pricing",
    about: "About",
    dashboard: "Dashboard",
    languageSwitcher: "Choose language",
    student: {
      dashboard: "Dashboard",
      findTeachers: "Find teachers",
      bookings: "My bookings",
      packages: "Packages",
      lessons: "Lessons",
      settings: "Settings",
    },
    teacher: {
      dashboard: "Dashboard",
      profile: "Profile",
      availability: "Availability",
      bookings: "Bookings",
      earnings: "Earnings",
      lessons: "Lessons",
      settings: "Settings",
    },
  },
  subjectNames: {
    mathematics: "Mathematics",
    "applied-mathematics": "Applied Mathematics",
    physics: "Physics",
    chemistry: "Chemistry",
    biology: "Biology",
    "computer-science": "Computer Science",
    german: "German",
    french: "French",
    italian: "Italian",
    english: "English",
    spanish: "Spanish",
    latin: "Latin",
    greek: "Greek",
    history: "History",
    geography: "Geography",
    economics: "Economics & Law",
    philosophy: "Philosophy",
    "pedagogy-psychology": "Pedagogy & Psychology",
    "religious-studies": "Religious Studies",
    "visual-arts": "Visual Arts",
    music: "Music",
    sports: "Sports",
  },
  status: {
    pending: "Pending",
    confirmed: "Confirmed",
    rejected: "Rejected",
    cancelled: "Cancelled",
    completed: "Completed",
  },
  auth: {
    welcomeBack: "Welcome back",
    signInSubtitle: "Sign in to your Academigo account",
    createAccount: "Create your account",
    signUpSubtitle: "Join Academigo as a student or teacher",
    email: "Email",
    password: "Password",
    fullName: "Full name",
    iAmA: "I am a",
    student: "Student",
    teacher: "Teacher",
    signingIn: "Signing in…",
    signIn: "Sign in",
    creatingAccount: "Creating account…",
    confirmPassword: "Confirm password",
    passwordMismatch: "Passwords do not match.",
    noAccount: "Don't have an account?",
    hasAccount: "Already have an account?",
    errors: {
      emailPasswordRequired: "Email and password are required.",
      invalidAccountType: "Invalid account type.",
      passwordTooShort: "Password must be at least 8 characters.",
    },
    verifyEmail: {
      title: "Verify your email",
      subtitle: "We've sent you a confirmation link.",
      instruction: "Please check your inbox and click the link to activate your account.",
      spam: "Can't find the link? Check your spam folder.",
      backToLogin: "Back to login",
    },
    forgotPassword: {
      title: "Forgot password",
      subtitle: "Enter your email address and we'll send you a reset link.",
      submit: "Send link",
      submitting: "Sending link…",
      successTitle: "Link sent",
      successMessage: "If an account with that email address exists, we've sent a reset link.",
      backToLogin: "Back to login",
      forgotPasswordLink: "Forgot password?",
    },
    updatePassword: {
      title: "Set new password",
      subtitle: "Enter your new password.",
      passwordLabel: "New password",
      submit: "Update password",
      submitting: "Updating…",
    },
  },
  home: {
    portal: {
      badge: "Academigo platform",
      title: "Personal support meets",
      titleHighlight: "digital learning guidance.",
      subtitle:
        "Find the right teacher among every subject we offer, book lessons, and learn with a platform that supports you between sessions.",
      learnMore: "Learn more on academigo.xyz",
      browseTeachers: "Browse teachers",
      footerTagline: "{subjects}. Online and in Zurich.",
      stepsSection: {
        eyebrow: "Getting started",
        title: "How registration works",
        description:
          "Follow the steps for your role — whether you want to learn or teach across all {count} subjects we offer.",
      },
      authAside: {
        eyebrow: "Academigo platform",
        title: "Structured tutoring, built for you.",
        body: "Book lessons, manage credits, and connect with reviewed tutors — all in one place.",
      },
      keyFacts: {
        eyebrow: "At a glance",
        title: "What you need to know",
        subjects: {
          title: "Subjects",
          desc: "Green badges show subjects with approved teachers you can book today. Gray badges are on our roadmap.",
          availableNow: "Teachers available",
          comingSoon: "Coming soon",
        },
        locations: {
          title: "Online & in Zurich",
          desc: "Video lessons or in-person meetings, whichever fits your schedule.",
        },
        pricing: {
          title: "Clear CHF packages",
          desc: "Single lessons or bundles with transparent pricing.",
        },
        teachers: {
          title: "Reviewed teachers",
          desc: "Every profile is checked by our team before going live.",
        },
      },
      student: {
        eyebrow: "For students",
        title: "How to get started",
        cta: "Sign up as a student",
        steps: [
          {
            title: "Create your account",
            description:
              "Sign up with your email, choose the student role, and set a password.",
          },
          {
            title: "Verify your email",
            description:
              "Click the confirmation link in your inbox to activate your account.",
          },
          {
            title: "Complete your profile",
            description:
              "Tell us your school level, subject, learning goals, and lesson preferences.",
          },
          {
            title: "Find teachers & book",
            description:
              "Browse approved tutors, buy lesson credits, and book sessions from your dashboard.",
          },
        ],
      },
      teacher: {
        eyebrow: "For teachers",
        title: "How to join Academigo",
        cta: "Apply as a teacher",
        steps: [
          {
            title: "Create your account",
            description:
              "Sign up with your email, choose the teacher role, and set a password.",
          },
          {
            title: "Verify your email",
            description:
              "Confirm your address via the link we send you before continuing.",
          },
          {
            title: "Build your profile",
            description:
              "Add your bio, education, experience, subjects, photo, and lesson formats.",
          },
          {
            title: "Wait for approval",
            description:
              "Our team reviews your profile. You are not visible to students until approved.",
          },
          {
            title: "Go live",
            description:
              "Once approved, set your availability and start receiving booking requests.",
          },
        ],
      },
      cta: {
        title: "Ready to begin?",
        subtitle:
          "Log in or create an account to find a teacher for any of our {count} subjects — {subjects}.",
      },
    },
    trust: {
      experience: "10+ years of experience",
      locations: "Online and in Zurich",
      support: "Individual support",
      platform: "Own learning platform",
      subjects: "22 school subjects",
    },
    subjects: {
      eyebrow: "Subjects",
      title: "All subjects we offer",
      description:
        "Green badges mean an approved teacher is ready to teach that subject. We cover the full Swiss school curriculum.",
      active: "Teachers available",
      soon: "Coming soon",
    },
  },
  teachers: {
    title: "Our teachers",
    subtitle:
      "Browse approved Academigo tutors. Every teacher is reviewed before appearing on the platform.",
    emptyTitle: "Teachers coming soon",
    emptyDesc:
      "We're onboarding our first verified tutors. Check back shortly or sign up as a teacher.",
    becomeTeacher: "Become a teacher",
    openings: {
      eyebrow: "Open engagements",
      title: "We're looking for you!",
      description:
        "We're expanding our tutoring offer and are hiring for the specific subject areas below. Each card describes one role — apply for the position that matches your expertise.",
      applyCta: "Apply for this role",
      footnote:
        "After sign-up you'll select the subjects you teach in your profile. We review each application individually before your profile goes live for students.",
      positions: [
        {
          id: "stem",
          badge: "Open position",
          roleTitle: "Tutoring teacher · STEM",
          subjects: "Mathematics, physics, chemistry, biology, computer science",
          description:
            "You support students in STEM subjects with structured 1:1 lessons — online or in Zurich.",
          requirements: [
            "Degree or Matura with strong grades in at least one STEM subject",
            "Tutoring or teaching experience preferred",
            "Available at least 2–4 hours per week",
          ],
        },
        {
          id: "languages",
          badge: "Open position",
          roleTitle: "Tutoring teacher · Languages",
          subjects: "German, French, English, Italian, Spanish, Latin, Greek",
          description:
            "You help with reading, writing, grammar, and exam prep in the languages you teach confidently.",
          requirements: [
            "Native level or C1 in at least one language we offer",
            "Experience with gymnasium or vocational school level",
            "Online lessons and optional in-person sessions in Zurich",
          ],
        },
        {
          id: "humanities",
          badge: "Open position",
          roleTitle: "Tutoring teacher · Geography & society",
          subjects: "Geography, history, economics & law, religious studies",
          description:
            "You guide students through society, space, and economics subjects with clear structure and explanations.",
          requirements: [
            "Completed or ongoing degree in a relevant field",
            "Confident explaining connections and exam material",
            "Patient, supportive approach with learners",
          ],
        },
        {
          id: "arts",
          badge: "Open position",
          roleTitle: "Tutoring teacher · Philosophy & creative subjects",
          subjects: "Philosophy, pedagogy & psychology, visual arts, music, sports",
          description:
            "You teach subjects outside STEM and languages — from text analysis to practical exercises.",
          requirements: [
            "Demonstrable qualification or strong performance in your subject",
            "Willingness to plan learning goals individually",
            "Profile with photo and short bio on Academigo",
          ],
        },
      ],
    },
  },
  teacherProfile: {
    about: "About",
    education: "Education",
    teachingStyle: "Teaching style",
    experience: "Experience",
    qualifications: "Qualifications",
    bookLesson: "Book a lesson",
    bookHint:
      "Scheduling opens in the next release. Sign up to purchase credits and book when availability goes live.",
    signUpToBook: "Sign up to book",
    viewPackages: "View packages",
    slotsComing: "Available time slots — coming in Phase 2",
    getCredits: "Get lesson credits",
  },
  subjects: {
    title: "Subjects",
    subtitle: "Expert tutoring across core STEM subjects — more coming soon.",
    availableNow: "Available now",
    comingSoon: "Coming soon",
    available: "Available",
    availableDesc: "Book lessons with verified Academigo teachers.",
    comingSoonDesc: "We are onboarding expert teachers for this subject.",
  },
  pricing: {
    title: "Academigo programmes",
    subtitle:
      "All prices in CHF. One-time purchases — no subscriptions, no commitments.",
    mostPopular: "Most popular",
    included: "Includes:",
    buyButton: "Buy now",
    perLesson: "per lesson",
    redirecting: "Redirecting…",
    tiers: {
      starter: {
        kindLabel: "Flexible",
        name: "Academigo Starter",
        tagline:
          "Ideal for students who need support with a specific topic or short-term exam preparation.",
        features: [
          "1 lesson credit",
          "Access to the Academigo platform",
          "Online or in-person lessons",
          "Progress tracking",
        ],
      },
      focus: {
        kindLabel: "Most popular",
        name: "Academigo Focus",
        tagline:
          "Ideal for students preparing for exams or systematically closing learning gaps.",
        savings: "Save CHF 20",
        features: [
          "5 lesson credits",
          "Access to the Academigo platform",
          "Online or in-person lessons",
          "Progress tracking",
        ],
      },
      excellence: {
        kindLabel: "Best value",
        name: "Academigo Excellence",
        tagline:
          "Ideal for long-term academic support and continuous improvement.",
        savings: "Save CHF 100",
        features: [
          "10 lesson credits",
          "Access to the Academigo platform",
          "Online or in-person lessons",
          "Progress tracking",
        ],
      },
    },
    faq: {
      title: "Frequently asked questions",
      items: [
        {
          q: "What is a credit?",
          a: "One credit equals one lesson (50 minutes). You book a session and the credit is deducted when the session is completed.",
        },
        {
          q: "Do my credits expire?",
          a: "No. Credits never expire — use them whenever it suits you.",
        },
        {
          q: "How do I book a lesson?",
          a: "After purchasing, log in, choose a teacher, and book directly in their calendar. The teacher confirms the booking.",
        },
        {
          q: "What payment methods are accepted?",
          a: "We accept all major credit and debit cards (Visa, Mastercard, American Express) as well as TWINT — securely processed via Stripe.",
        },
        {
          q: "Can I buy multiple packages?",
          a: "Yes. Your credits are pooled together and never expire, so you can buy additional packages at any time.",
        },
      ],
    },
  },
  chat: {
    title: "Academigo Assistant",
    intro: "Hi! I can answer questions about our offer, pricing, and how it works. How can I help?",
    placeholder: "Ask a question…",
    send: "Send",
    open: "Open chat",
    close: "Close chat",
    error: "Something went wrong. Please try again or message us on WhatsApp.",
    whatsapp: "Continue on WhatsApp",
    whatsappPrefill: "Hi Academigo! I have a question:",
  },
  about: {
    title: "About Academigo",
    missionTitle: "Our mission",
    missionText:
      "Academigo connects Swiss students with exceptional tutors in Mathematics, Physics, and Chemistry. We believe every student deserves personalized support, clear goals, and a trustworthy platform.",
    teachersTitle: "For teachers",
    teachersText:
      "Join our network of educators, set your availability, and grow your tutoring practice with transparent earnings and professional tools.",
  },
  student: {
    onboarding: {
      title: "Welcome to Academigo",
      subtitle:
        "Tell us about your learning goals so we can recommend the right teachers.",
      submit: "Complete onboarding",
      saving: "Saving…",
      schoolLevel: "School level",
      selectLevel: "Select level",
      subjectNeeded: "Subject you need help with",
      selectSubject: "Select subject",
      learningGoal: "Learning goal",
      learningGoalPlaceholder: "e.g. Pass Gymnasium exam",
      preferredLanguage: "Preferred lesson language",
      languagePlaceholder: "German, English, …",
      modality: "Online or in-person",
      selectModality: "Select preference",
      modalityOnline: "Online only",
      modalityInPerson: "In-person only",
      modalityBoth: "Both",
      difficulties: "Current difficulties (optional)",
      difficultiesPlaceholder: "What topics are challenging you right now?",
      schoolLevels: [
        "Primary",
        "Secondary I",
        "Secondary II / Gymnasium",
        "University",
        "Other",
      ],
      errors: {
        required: "Please fill in all required fields.",
        saveFailed:
          "Your information could not be saved. Please try again or contact support.",
      },
    },
    dashboard: {
      greeting: "Hello, {name}",
      subtitle: "Your overview — credits, sessions, and matching teachers.",
      availableCredits: "Available credits",
      buyPackages: "Buy credits",
      plannedLessons: "Planned lessons",
      upcomingLessons: "Next lessons",
      recommended: "Teachers",
      upcomingTitle: "Next lessons",
      noUpcoming:
        "When you book a lesson, it will show up here. You can buy credits anytime under Packages.",
      noUpcomingHint: "Booking will be enabled shortly.",
      recommendedTitle: "Teachers for you",
      noTeachers:
        "Once teachers are approved, they appear here — based on your onboarding.",
      activePlan: "Active plan",
      purchasedOn: "Purchased on {date}",
      viewPlan: "View packages",
      pendingReviewBanner: "You have {count} completed session(s) without a review. Your feedback helps teachers improve.",
      pendingReviewAction: "Leave reviews →",
    },
    teachers: {
      title: "Find teachers",
      subtitle: "Browse approved tutors",
      emptyTitle: "No teachers yet",
      emptyDesc: "We're reviewing teacher applications. Check back soon.",
    },
    bookings: {
      title: "My bookings",
      subtitle: "Upcoming and past lessons",
      emptyTitle: "No bookings yet",
      emptyDesc:
        "Confirmed sessions will appear here once scheduling goes live.",
      emptyHint: "Get ready: buy credits and pick a teacher.",
      findTeachers: "View teachers",
      creditsHint: "Each session (50 min.) costs 1 credit",
      upcoming: "Upcoming",
      pastSessions: "Past Sessions",
      creditHistory: "Credit history",
      creditHistoryEmpty: "No completed sessions yet.",
      creditDeducted: "−{n} credit",
    },
    packages: {
      title: "Lesson packages",
      subtitle: "Purchase credits to book lessons",
      stripeNote:
        "Online checkout with Stripe will be enabled in Phase 3. Review packages below or contact us for early access.",
      viewDetails: "View details",
      creditsHint: "1 credit = 1 lesson (50 min)",
      paymentSuccess: "Payment successful! Your credits have been added to your account.",
      paymentCancelled: "Payment cancelled. No charges were made.",
      currentPlan: "Your plan",
      active: "Active",
      oneTimePurchase: "One-time purchase",
      purchasedOn: "Purchased on {date}",
      billingHistory: "Billing history",
      hideBillingHistory: "Hide billing history",
      noPayments: "No payment history yet.",
      creditsLabel: "Credits",
      ledgerTitle: "Credit history",
      ledgerEmpty: "No credit activity yet. Purchase a package to get started.",
      ledgerSessionWith: "Session with {name}",
      ledgerBalance: "Balance",
    },
    settings: {
      title: "Settings",
      subtitle: "Account details",
      name: "Name",
      email: "Email",
      role: "Role",
      preferredSubject: "Preferred subject",
      noSubject: "No subject selected",
      save: "Save",
      saving: "Saving…",
      saved: "Saved",
    },
  },
  teacher: {
    onboarding: {
      title: "Build your teacher profile",
      subtitle:
        "Complete your profile for admin review. Once approved, students can find and book you.",
      submit: "Submit profile for review",
      submitting: "Submitting…",
      profilePicture: "Profile picture",
      subjectsTeach: "Subjects you teach",
      offersOnline: "Offer online lessons",
      offersInPerson: "Offer in-person lessons",
      location: "Location (if in-person)",
      locationPlaceholder: "Zurich, etc.",
      languages: "Languages you teach in",
      languageOther: "Other languages",
      languageOtherPlaceholder: "e.g. Portuguese, Arabic (comma-separated)",
      experienceOptions: {
        less1: "Less than 1 year",
        one2: "1–2 years",
        three5: "3–5 years",
        six10: "6–10 years",
        ten_plus: "More than 10 years",
      },
      payoutTitle: "Payout information",
      payoutOptionalNote: "Optional — you can fill this in later when requesting a payout.",
      payoutName: "Full name (as on bank account)",
      payoutIban: "IBAN",
      payoutIbanPlaceholder: "CH56 0483 5012 3456 7800 9",
      payoutStreet: "Street and number",
      payoutStreetPlaceholder: "Musterstrasse 1",
      payoutZip: "Postcode",
      payoutZipPlaceholder: "8001",
      payoutCity: "City",
      payoutCityPlaceholder: "Zurich",
      payoutTwint: "Twint phone number",
      payoutTwintPlaceholder: "+41 79 123 45 67",
      subjectsRequired: "Please select at least one subject.",
      languagesRequired: "Please select at least one language.",
      defaultMeetLink: "Google Meet link",
      defaultMeetLinkHint:
        "You can add this later from your settings.",
      errors: {
        required:
          "Please complete all required fields and select at least one subject.",
        languagesRequired: "Please select at least one language.",
        cvUploadFailed:
          "Your CV could not be uploaded. Try a smaller file or continue without a CV.",
        saveFailed:
          "Your profile could not be saved. Please try again or contact support.",
        avatarUploadFailed:
          "Your profile photo could not be uploaded. Try a smaller image or continue without a photo.",
      },
    },
    dashboard: {
      greeting: "Welcome, {name}",
      subtitle: "Requests, sessions, and profile — in one place.",
      deactivatedBanner:
        "Your account is inactive. You are no longer visible to students. Please contact support if you believe this is a mistake.",
      reviewBanner:
        "Your profile is under review. After approval, you will be visible to students on Academigo.",
      profileCompletion: "Profile completion",
      pendingRequests: "Pending requests",
      viewRequests: "View requests",
      upcomingLessons: "Upcoming lessons",
      upcomingTitle: "Upcoming lessons",
      noUpcoming: "No confirmed lessons scheduled yet.",
      completed: "Completed",
      yourRating: "Your rating",
      noRatingYet: "—",
      reviewsCount: "{count} review(s)",
      earningsThisMonth: "Earnings this month",
      requestPayout: "Request payout",
      viewEarnings: "View earnings →",
      levelTitle: "Your teacher level",
      levelJunior: "Junior Teacher",
      levelAcademigoTeacher: "Academigo Teacher",
      levelVerified: "Verified Teacher",
      nextLevelTitle: "Path to next level",
      completedSessions: "Completed sessions",
      averageRating: "Average rating",
      nextLevelRequirements: "Complete {sessions} sessions and maintain a {rating}★ average to apply for the next level. Contact us when you are ready.",
      alreadyTopLevel: "You have reached the highest teacher level. Thank you for your commitment!",
      levelRate: "CHF {rate} / class",
      nextLevelRate: "Next level: CHF {rate} / class",
      learnMoreLevels: "Learn more about levels & FAQ",
      status: "Status",
      earningsTitle: "Earnings",
      earningsNote: "View your full earnings history and request payouts.",
      pendingEvalBanner: "You have {count} completed session(s) where you have not yet added private notes. Session notes are required.",
      pendingEvalAction: "Complete evaluations →",
      completeProfile: "Complete profile →",
      viewProfile: "View profile →",
      missingFieldsLabel: "Missing:",
      profileFields: {
        bio: "Bio",
        education: "Education",
        experience: "Experience",
        teaching_style: "Teaching style",
        payout_info_placeholder: "Payout info",
      },
      pending: {
        title: "Your application is under review",
        stepsTitle: "What happens next",
        stepReceived: "Application received — thank you for applying!",
        stepReview:
          "The Academigo team reviews your profile, subjects, and motivation letter.",
        stepEmail:
          "You will receive an email once your profile is approved and visible to students.",
        summaryTitle: "Your application summary",
        summarySubjects: "Subjects",
        summaryLanguages: "Languages",
        summaryModality: "Lesson format",
        modalityOnline: "Online",
        modalityInPerson: "In person",
        modalityBoth: "Online & in person",
        editProfileCta: "Edit profile",
        faqCta: "Teacher FAQ",
      },
    },
    profile: {
      title: "My profile",
      subtitle: "Your public teacher profile",
      completeOnboarding: "Complete onboarding first.",
      subjects: "Subjects",
      modality: "Format",
      save: "Save",
      saving: "Saving…",
      saved: "Saved",
    },
    availability: {
      title: "Availability",
      subtitle: "Manage your teaching schedule",
      phase2:
        "Weekly recurring slots, specific dates, and unavailable dates will be available in Phase 2. Default lesson duration: 50 minutes.",
      addTitle: "Add availability",
      addDesc:
        "Set weekly recurring time windows when you are available to teach. Lessons are 50 minutes. Students can book slots at 15-minute increments within your ranges.",
      currentTitle: "Current availability",
      noRanges: "No availability set yet. Add a range above to get started.",
      blockedTitle: "Blocked dates",
      blockedDesc:
        "Block specific dates when you are unavailable, even if they fall within a weekly range.",
      remove: "Remove",
      allDay: "all day",
      formDay: "Day",
      formFrom: "From",
      formTo: "To",
      formAdding: "Adding…",
      formAddRange: "Add range",
      formDate: "Date",
      formOptional: "(optional)",
      formLeaveBlank: "Leave start/end blank to block the entire day.",
      formBlocking: "Blocking…",
      formBlock: "Block",
      saveSchedule: "Save schedule",
      saving: "Saving…",
      scheduleSaved: "Schedule saved.",
      addSlot: "Add time slot",
      dayOff: "Off",
      dayLabels: [
        "Sunday",
        "Monday",
        "Tuesday",
        "Wednesday",
        "Thursday",
        "Friday",
        "Saturday",
      ],
    },
    bookings: {
      title: "Bookings",
      subtitle: "Requests and scheduled lessons",
      emptyTitle: "No open requests",
      emptyDesc:
        "New booking requests from students will appear here — accept or decline.",
      emptyHint: "Make sure your profile is complete and approved.",
      requests: "Requests",
      pendingEmptyTitle: "No pending requests",
      pendingEmptyDesc: "New booking requests from students will appear here.",
      upcoming: "Upcoming",
      upcomingEmptyTitle: "No upcoming sessions",
      upcomingEmptyDesc: "Confirmed upcoming sessions will appear here.",
      pastSessions: "Past Sessions",
      pastEmptyTitle: "No past sessions",
      pastEmptyDesc: "Completed and cancelled sessions will appear here.",
    },
    settings: {
      title: "Settings",
      subtitle: "Account details",
      name: "Name",
      email: "Email",
      payoutTitle: "Payout information",
      payoutOptionalNote: "Optional — you can fill this in later when requesting a payout.",
      payoutName: "Full name (as on bank account)",
      payoutIban: "IBAN",
      payoutIbanPlaceholder: "CH56 0483 5012 3456 7800 9",
      payoutStreet: "Street and number",
      payoutStreetPlaceholder: "Musterstrasse 1",
      payoutZip: "Postcode",
      payoutZipPlaceholder: "8001",
      payoutCity: "City",
      payoutCityPlaceholder: "Zurich",
      payoutTwint: "Twint phone number",
      payoutTwintPlaceholder: "+41 79 123 45 67",
      save: "Save",
      saving: "Saving…",
      saved: "Saved",
    },
  },
  errors: {
    invalidAccountType: "Invalid account type.",
    onboardingRequired: "Please fill in all required fields.",
    unauthorized: "Unauthorized.",
    teacherNotFound: "Teacher profile not found.",
  },
  admin: {
    nav: {
      dashboard: "Dashboard",
      teachers: "Teachers",
      students: "Students",
      bookings: "Bookings",
      promotions: "Promotions",
      payouts: "Payouts",
      revenue: "Revenue",
      analytics: "Teacher Analytics",
      missingLinks: "Missing Meet Links",
      sessions: "Sessions",
      settings: "Settings",
    },
    dashboard: {
      title: "Admin",
      subtitle: "Platform overview",
      statTeachers: "Total teachers",
      statStudents: "Total students",
      statBookings: "Total bookings",
      statRevenue: "Revenue CHF",
      revenueStub: "— (Phase 3)",
      needsAttention: "Needs attention",
      pendingTeachers: "Pending approvals",
      pendingPromotions: "Pending promotions",
      pendingPayouts: "Pending payouts",
      viewTeachers: "View teachers",
      viewPromotions: "Review promotions",
      viewPayouts: "Review payouts",
    },
    teachers: {
      title: "Teachers",
      colName: "Name",
      colEmail: "Email",
      colLevel: "Level",
      colStatus: "Status",
      colJoined: "Joined",
      colActions: "Actions",
      approve: "Approve",
      statusApproved: "Approved",
      statusPending: "Pending",
      statusActive: "Active",
      statusInactive: "Deactivated",
      deactivate: "Deactivate",
      activate: "Reactivate",
      empty: "No teachers found.",
      detail: {
        back: "Back to teachers",
        joined: "Joined",
        bio: "Bio",
        education: "Education",
        experience: "Experience",
        teachingStyle: "Teaching style",
        subjects: "Subjects",
        languages: "Languages",
        format: "Format",
        online: "Online",
        inPerson: "In-person",
        location: "Location",
        payoutInfo: "Payout info",
        noPayoutInfo: "Not provided",
        notProvided: "—",
        approveAction: "Approve teacher",
        alreadyApproved: "This teacher is already approved.",
        deactivateAction: "Deactivate teacher",
        activateAction: "Reactivate teacher",
        teacherInactive: "This teacher is deactivated and hidden from public listings.",
        teacherActive: "This teacher is active and visible when approved.",
      },
    },
    students: {
      title: "Students",
      colName: "Name",
      colEmail: "Email",
      colCredits: "Credits",
      colJoined: "Joined",
      empty: "No students found.",
    },
    bookings: {
      title: "Bookings",
      colStudent: "Student",
      colTeacher: "Teacher",
      colDate: "Date",
      colCredits: "Credits",
      colStatus: "Status",
      tabs: {
        all: "All",
        pending: "Pending",
        confirmed: "Confirmed",
        completed: "Completed",
        cancelled: "Cancelled",
      },
      empty: "No bookings found.",
    },
    promotions: {
      title: "Promotions",
      colTeacher: "Teacher",
      colCurrentLevel: "Current level",
      colRequestedLevel: "Requested level",
      colDate: "Date",
      colStatus: "Status",
      colActions: "Actions",
      review: "Review",
      approve: "Approve",
      reject: "Reject",
      notePlaceholder: "Optional note…",
      empty: "No pending promotions.",
    },
    payouts: {
      title: "Payouts",
      colTeacher: "Teacher",
      colAmount: "Amount CHF",
      colStatus: "Status",
      colDate: "Date",
      colActions: "Actions",
      markProcessed: "Mark processed",
      statusPending: "Pending",
      statusProcessed: "Processed",
      empty: "No pending payouts.",
      viewDetails: "Details",
      close: "Close",
      dialogTitle: "Payout details",
      dialogTeacher: "Teacher",
      dialogEmail: "Email",
      dialogAmount: "Amount",
      dialogRequested: "Requested on",
      dialogSessions: "Included sessions",
      dialogColDate: "Date",
      dialogColStudent: "Student",
      dialogColSubject: "Subject",
      dialogColAmount: "Amount CHF",
      dialogNoSessions: "No linked sessions found.",
      confirmPayout: "Confirm payout & send email",
      confirmPayoutLoading: "Processing…",
      dialogBankDetails: "Bank details",
      dialogPayoutName: "Account holder",
      dialogPayoutIban: "IBAN",
      dialogPayoutAddress: "Address",
      dialogPayoutTwint: "TWINT",
      dialogNoBankDetails: "No bank details on file.",
    },
    missingLinks: {
      title: "Missing Meet Links",
      subtitle: "Upcoming confirmed lessons without a meeting link",
      noMissingLinks: "All confirmed lessons have a meeting link.",
      colDate: "Date",
      colTeacher: "Teacher",
      colStudent: "Student",
      colHoursUntil: "Time until",
    },
    sessions: {
      title: "Session Feedback",
      subtitle: "Completed sessions — teacher evaluations and student reviews",
      colDate: "Date",
      colParticipants: "Student / Teacher",
      colTeacherRating: "Teacher rating",
      colTeacherNotes: "Teacher notes",
      colStudentReview: "Student review",
      noRating: "—",
      noNotes: "—",
      noReview: "No review yet",
      empty: "No completed sessions yet.",
    },
    analytics: {
      title: "Teacher Analytics",
      subtitle: "Per-teacher revenue attribution and margin breakdown",
      viewAnalytics: "Teacher analytics →",
      periodLabel: "Period",
      periodAll: "All time",
      periodThisMonth: "This month",
      periodLastMonth: "Last month",
      periodLast30: "Last 30 days",
      periodLast90: "Last 90 days",
      periodYtd: "Year to date",
      statSessions: "Sessions",
      statRevenue: "Student revenue CHF",
      statPayout: "Teacher payouts CHF",
      statMargin: "Platform margin CHF",
      wacNote: "Revenue attributed at WAC of CHF {wac}/credit (all-time avg)",
      colName: "Teacher",
      colLevel: "Level",
      colSessions: "Sessions",
      colRating: "Avg rating",
      colReviews: "Reviews",
      colStudentRevenue: "Student revenue",
      colTeacherPayout: "Teacher payout",
      colMargin: "Platform margin",
      colMarginPct: "Margin %",
      noRating: "—",
      empty: "No completed sessions in this period.",
      levelJunior: "Junior",
      levelAcademigoTeacher: "Academigo",
      levelVerified: "Verified",
    },
    revenue: {
      title: "Platform Revenue",
      subtitle: "Gross student payments minus teacher payouts",
      statGross: "Gross revenue CHF",
      statPayouts: "Teacher payouts CHF",
      statMargin: "Platform margin CHF",
      tableTitle: "Monthly breakdown",
      colMonth: "Month",
      colRevenue: "Student revenue",
      colPayouts: "Teacher payouts",
      colMargin: "Margin",
      empty: "No transactions yet.",
    },
    settings: {
      title: "Platform Settings",
      subtitle: "Configure tier rates and promotion requirements",
      tierSection: "Teacher Tier Rates",
      juniorRate: "Junior rate (CHF per class)",
      midRate: "Academigo rate (CHF per class)",
      topRate: "Verified rate (CHF per class)",
      promotionSection: "Promotion Requirements",
      juniorSessionsRequired: "Sessions required (Junior → Academigo)",
      juniorRatingRequired: "Min. rating (Junior → Academigo)",
      midSessionsRequired: "Sessions required (Academigo → Verified)",
      midRatingRequired: "Min. rating (Academigo → Verified)",
      save: "Save settings",
      saving: "Saving…",
      saved: "Settings saved.",
    },
  },
  bookings: {
    statusPending: "Pending",
    statusConfirmed: "Confirmed",
    statusCompleted: "Completed",
    statusCancelled: "Cancelled",
    confirm: "Confirm",
    decline: "Decline",
    markComplete: "Mark as Done",
    cancel: "Cancel",
    sessionEvalTitle: "How did the session go?",
    sessionEvalSubtitle: "Your rating is private. Session notes are required and shared with your student.",
    ratingExcellent: "Excellent",
    ratingGood: "Good",
    ratingChallenging: "Challenging",
    ratingNoShow: "No-show",
    teacherNotesLabel: "Session notes",
    teacherNotesPlaceholder: "Topics covered, follow-up for next session…",
    completeSession: "Complete session",
    backToActions: "Back",
    sessionNotesForStudent: "Session notes (shared with student)",
    editNotes: "Edit notes",
    addNotes: "Add notes",
    saveNotes: "Save notes",
    notesSaved: "Notes saved.",
    notesFromTeacher: "Notes from your teacher",
    ratingRequired: "Please select a session rating before completing.",
    noBookings: "No bookings yet",
    topic: "Notes:",
    meetLink: "Meet Link:",
    joinLesson: "Join Lesson",
    waitingForTeacher: "Waiting for teacher",
    downloadCalendar: "Download calendar",
    meetLinkAdded: "Meet Link Added",
    meetLinkMissing: "Meet Link Missing",
    addMeetLink: "Add Meet Link",
    updateMeetLink: "Update Meet Link",
    meetLinkPlaceholder: "https://meet.google.com/…",
    confirmBookingTitle: "Confirm booking",
    confirming: "Confirming…",
    declineTitle: "Decline request",
    declining: "Declining…",
    cancelSession: "Cancel session",
    cancelling: "Cancelling…",
    cancelBooking: "Cancel booking",
    noMeetLinkWarning:
      "No default Meet link set. You must provide a Meet link when confirming.",
    settingsLink: "Go to Settings to set a default link.",
    meetLinkFieldLabel: "Google Meet link",
    update: "Update",
    saving: "Saving…",
    addLink: "Add link",
    downloadIcs: "Download .ics",
    creditReserved: "1 credit reserved",
    creditsLabel: "Credits:",
    earnedLabel: "Earned:",
    addToCalendar: "Add to calendar",
    bookingCancelled: "This booking was cancelled.",
    pendingReview: "Pending review",
    loadingSlots: "Loading slots…",
    noSlotsForDay: "No available slots for this day.",
    availableTimesOn: "Available times on {date}",
    slotUnavailable: "Not available",
    requesting: "Requesting…",
    requestBooking: "Request booking",
    requestedTitle: "Booking requested!",
    requestedDesc:
      "Your slot is reserved while {teacherName} reviews your request. You will receive a confirmation email once confirmed.",
    selectedSlot: "Selected slot",
    withTeacher: "with {name}",
    subjectLabel: "Subject(s)",
    subjectOptional: "(optional)",
    topicLabel: "Additional notes",
    topicOptional: "(optional)",
    topicPlaceholder:
      "e.g. Quadratic equations, exam prep for Matura, grammar review…",
  },
  reviews: {
    leaveReview: "Leave a review",
    feedbackTitle: "Rate your session",
    feedbackSubtitle: "Your feedback helps improve the experience.",
    submitReview: "Submit review",
    ratingRequired: "Please select a rating (1–5 stars).",
    reviewSubmitted: "Thank you for your feedback!",
    alreadyReviewed: "You've already reviewed this session.",
    comment: "Comment (optional)",
    commentPlaceholder: "How was the lesson?",
    noReviewsYet: "No reviews yet.",
    reviewsTitle: "Reviews",
    submitting: "Submitting…",
  },
  earnings: {
    title: "Earnings",
    subtitle: "Your earnings and payouts",
    pendingBalance: "Pending balance",
    totalEarned: "Total earned",
    requestPayout: "Request payout",
    payoutRequested: "Payout requested",
    earningsHistory: "Earnings history",
    noEarnings: "No earnings yet.",
    session: "Session",
    amount: "Amount",
    colDate: "Date",
    colStudent: "Student",
    colAmount: "Amount CHF",
    payoutPending: "Payout pending",
    payoutProcessed: "Payout processed",
    payoutHistory: "Payout history",
    colPayoutDate: "Date",
    colPayoutAmount: "Amount",
    colPayoutStatus: "Status",
    noPayouts: "No payouts yet.",
    noBankDetails: "Bank details missing",
    noBankDetailsHint: "Please add your bank details in Settings before requesting a payout.",
    goToSettings: "Go to settings",
    reviewsTitle: "Student reviews",
    noReviews: "No reviews yet. Complete sessions and students will leave feedback here.",
  },
  lessons: {
    teacherTitle: "My Lessons",
    studentTitle: "My Lessons",
    activeStudents: "Active Students",
    noActiveStudents: "No active students yet.",
    recurringSchedules: "Recurring Schedules",
    noSchedules: "No recurring schedules.",
    upcomingLessons: "Upcoming Lessons",
    noUpcomingLessons: "No upcoming lessons.",
    rescheduleRequests: "Reschedule Requests",
    noRescheduleRequests: "No reschedule requests.",
    pauseSchedule: "Pause",
    resumeSchedule: "Resume",
    cancelSchedule: "Cancel",
    markComplete: "Mark Complete",
    approveReschedule: "Approve",
    rejectReschedule: "Reject",
    weekdays: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"] as const,
    creditBalance: "Credits",
    proposedTime: "Proposed time",
  },
  wallet: {
    title: "Credit Wallet",
    balance: "Available Balance",
    history: "Transaction History",
    noTransactions: "No transactions yet.",
    purchase: "Credit purchase",
    completion_deduction: "Lesson completed",
    cancellation_refund: "Lesson cancelled",
    admin_grant: "Admin adjustment",
  },
  schedules: {
    createTitle: "Set Up Recurring Lesson",
    weekdayLabel: "Day of week",
    startTimeLabel: "Start time",
    endTimeLabel: "End time",
    teacherLabel: "Teacher",
    createButton: "Create Schedule",
    activeStatus: "Active",
    pausedStatus: "Paused",
    cancelledStatus: "Cancelled",
  },
  reschedule: {
    requestTitle: "Request Reschedule",
    proposedDateLabel: "New date and time",
    submitButton: "Submit Request",
    pendingBadge: "Reschedule Pending",
  },
  calendar: {
    prev: "← Prev",
    next: "Next →",
    loading: "Loading…",
    available: "Available",
    selected: "Selected",
    dayHeaders: ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"],
    monthNames: [
      "January",
      "February",
      "March",
      "April",
      "May",
      "June",
      "July",
      "August",
      "September",
      "October",
      "November",
      "December",
    ],
  },
};
